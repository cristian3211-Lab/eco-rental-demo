    ...
    <div className="pt-24 pb-20">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 mt-10">
        ...
        <p className="text-sm text-gray-500 mb-6">
          Mostrando {sorted.length} de {machines.length} equipos
        </p>
        ...
        {sorted.length === 0 ? (
          <div className="rounded-xl border-2 border-dashed border-gray-300 p-12 text-center">
            ...
            <button
              onClick={() => { setCategory("Todas"); setBrand("Todas"); setSearch(""); setSortBy("default"); }}
            ...
            <AnimatePresence mode="popLayout">
              {sorted.map((machine) => (
                <motion.article
                  ...
                  className="group rounded-2xl border border-gray-200 overflow-hidden transition-shadow hover:shadow-lg hover:border-primary/30"
                >
                  {/* Image placeholder */}
                  <div className={`${machine.image} h-48 flex items-center justify-center relative`}>
                    ...
                    {/* Availability badge */}
                    {machine.available ? (
                      <span className="absolute top-3 right-3 inline-flex items-center gap-1.5 rounded-full bg-white/90 backdrop-blur-sm px-2.5 py-1 text-xs font-medium text-green-700 shadow-sm">
                        <span className="h-2 w-2 rounded-full bg-green-500" />
                        Disponible
                      </span>
                    ) : (
                      <span className="absolute top-3 right-3 inline-flex items-center gap-1.5 rounded-full bg-white/90 backdrop-blur-sm px-2.5 py-1 text-xs font-medium text-red-700 shadow-sm">
